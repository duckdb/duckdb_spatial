...
void CoreScalarFunctions::RegisterStContains(ClientContext &context) {
	...
	// POLYGON_2D - POINT_2D
	contains_function_set.AddFunction(ScalarFunction({GeoTypes::POLYGON_2D, GeoTypes::POINT_2D}, LogicalType::BOOLEAN, PolygonContainsPointFunction));
	within_function_set.AddFunction(ScalarFunction({GeoTypes::POINT_2D, GeoTypes::POLYGON_2D}, LogicalType::BOOLEAN, PointWithinPolygonFunction));
