...
// Polygon Area (COLUMN)
//----------------------------------------------------------------------
static void AreaFunction_2D_C(DataChunk &args, ExpressionState &state, Vector &result) {
    D_ASSERT(args.data.size() == 1);
    ...
    auto polygon_entries = ListVector::GetData(input);
...
void LayoutBenchmark::Register(ClientContext &context) {
    ...
    CreateScalarFunctionInfo create_polygon_2d_c_info(ScalarFunction(
        "geo_create_polygon2d_c", {GeoTypes::WKB_BLOB}, GEO_POLYGON_2D_C, CreatePolygon_2D_C));
	...
	CreateScalarFunctionInfo distance_2d_c_info(ScalarFunction(
	    "geo_distance2d_c", {GEO_POINT_2D_C, GEO_POINT_2D_C}, LogicalType::DOUBLE, Distance2DFunction_C));
    ...
    CreateScalarFunctionInfo polygon_area_2d_c_info(ScalarFunction(
        "geo_polygon_area2d_c", {GEO_POLYGON_2D_C}, LogicalType::DOUBLE, AreaFunction_2D_C));
    ...
    CreateScalarFunctionInfo create_line_2d_r_info(ScalarFunction(
        "geo_create_line2d_r", {GeoTypes::WKB_BLOB}, GEO_LINE_2D_R, CreateLine_2D_R));
...
//D CREATE TABLE t1 as SELECT geo_create_polygon2d_c(wkb_geometry) as geom, geo_create_point2d_c(st_x(st_centroid(wkb_geometry)), st_y(st_centroid(wkb_geometry))) as centroid FROM st_read('./geo/test/data/germany/forest/forest.fgb') LIMIT 1000;