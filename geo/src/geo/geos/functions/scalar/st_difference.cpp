...
static void DifferenceFunction(DataChunk &args, ExpressionState &state, Vector &result) {
	...
	BinaryExecutor::Execute<string_t, string_t, string_t>(args.data[0], args.data[1], result, args.size(), [&](string_t left, string_t right) {
		...
		auto geos_result = left_geos_geom.Difference(right_geos_geom);
...
void GEOSScalarFunctions::RegisterStDifference(ClientContext &context) {
	...
	set.AddFunction(ScalarFunction({GeoTypes::GEOMETRY(), GeoTypes::GEOMETRY()}, GeoTypes::GEOMETRY(), DifferenceFunction, nullptr, nullptr, nullptr, GEOSFunctionLocalState::Init));
