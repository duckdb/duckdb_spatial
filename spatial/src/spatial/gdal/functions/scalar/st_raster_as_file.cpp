...
static void RasterAsFileFunction_01(DataChunk &args, ExpressionState &state, Vector &result) {
	...
	TernaryExecutor::Execute<uintptr_t, string_t, string_t, bool>(args.data[0], args.data[1], args.data[2], result, args.size(),
...
static void RasterAsFileFunction_02(DataChunk &args, ExpressionState &state, Vector &result) {
	...
	GenericExecutor::ExecuteQuaternary<POINTER_TYPE, STRING_TYPE, STRING_TYPE, LIST_TYPE, BOOL_TYPE>(p1, p2, p3, p4, result, args.size(),
...
void GdalScalarFunctions::RegisterStRasterAsFile(DatabaseInstance &db) {
	...
	set.AddFunction(ScalarFunction({GeoTypes::RASTER(), LogicalType::VARCHAR, LogicalType::VARCHAR},
	                               LogicalType::BOOLEAN,
	                               RasterAsFileFunction_01));

	set.AddFunction(ScalarFunction({GeoTypes::RASTER(), LogicalType::VARCHAR, LogicalType::VARCHAR, LogicalType::LIST(LogicalType::VARCHAR)},
	                               LogicalType::BOOLEAN,
