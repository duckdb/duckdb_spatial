...
static void CollectionExtractTypeFunction(DataChunk &args, ExpressionState &state, Vector &result) {
    ...
    BinaryExecutor::Execute<string_t, int32_t, string_t>(input, dim, result, count, [&](string_t input, int32_t requested_type) {
        ...
        switch(requested_type) {
            ...
            default:
                throw InvalidInputException("Invalid requested type parameter for collection extract, must be 1 (POINT), 2 (LINESTRING) or 3 (POLYGON)");
        }
    });
}
...
static void CollectionExtractAutoFunction(DataChunk &args, ExpressionState &state, Vector &result) {
    ...
    UnaryExecutor::Execute<string_t, string_t>(input, result, count, [&](string_t input) {
        ...
        if(geometry.Type() == GeometryType::GEOMETRYCOLLECTION) {
            ...
            auto dim = collection.Aggregate([](Geometry &geom, int32_t d) { 
                return geom.IsEmpty() ? d : std::max(geom.Dimension(), d);
            }, 0);
...
void CoreScalarFunctions::RegisterStCollectionExtract(ClientContext &context) {
	...
	set.AddFunction(ScalarFunction({GeoTypes::GEOMETRY()}, GeoTypes::GEOMETRY(), CollectionExtractAutoFunction,
	                               nullptr, nullptr, nullptr, GeometryFunctionLocalState::Init));
    set.AddFunction(ScalarFunction({GeoTypes::GEOMETRY(), LogicalType::INTEGER}, GeoTypes::GEOMETRY(), CollectionExtractTypeFunction,
                                   nullptr, nullptr, nullptr, GeometryFunctionLocalState::Init));
