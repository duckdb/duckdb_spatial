...
struct GeometryFromWKTBindData : public FunctionData { 
	...
	explicit GeometryFromWKTBindData(bool ignore_invalid) 
		: ignore_invalid(ignore_invalid) {
	}

public:
	unique_ptr<FunctionData> Copy() const override {
		return make_unique<GeometryFromWKTBindData>(ignore_invalid);
	}
...
static void GeometryFromWKTFunction(DataChunk &args, ExpressionState &state, Vector &result) {
	...
	UnaryExecutor::ExecuteWithNulls<string_t, string_t>(input, result, count, [&](string_t &wkt, ValidityMask &mask, idx_t idx) {
...
static unique_ptr<FunctionData> GeometryFromWKTBind(ClientContext &context, ScalarFunction &bound_function, vector<unique_ptr<Expression>> &arguments) {
	if(arguments.empty()) {
	...
	for(idx_t i = 1; i < arguments.size(); i++) {
		...
		if (!arg->IsFoldable()) {
			throw InvalidInputException("Non-constant arguments are not supported in ST_GeomFromText optional arguments");
		}
...
void GEOSScalarFunctions::RegisterStGeomFromText(ClientContext &context) {
	...
	set.AddFunction(ScalarFunction({LogicalType::VARCHAR}, core::GeoTypes::GEOMETRY(), GeometryFromWKTFunction, GeometryFromWKTBind, nullptr, nullptr, GEOSFunctionLocalState::Init));
	set.AddFunction(ScalarFunction({LogicalType::VARCHAR, LogicalType::BOOLEAN}, core::GeoTypes::GEOMETRY(), GeometryFromWKTFunction, GeometryFromWKTBind, nullptr, nullptr, GEOSFunctionLocalState::Init));
