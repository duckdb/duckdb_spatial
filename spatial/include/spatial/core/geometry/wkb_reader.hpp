...
public:
    ...
    WKBReader(GeometryFactory &factory, const char *data, uint32_t length)
        : factory(factory), data(data), length(length), cursor(0), geom_has_z(false), geom_has_m(false),
geom_has_z_set(false), geom_has_m_set(false) {
    }
...
private:
	...
	double ReadDouble() {
		if (ORDER == WKBByteOrder::NDR) {
			...
			// swap bytes
			data = (data >> 56) | ((data >> 40) & 0xFF00) | ((data >> 24) & 0xFF0000) | ((data >> 8) & 0xFF000000) |
			       ((data << 8) & 0xFF00000000) | ((data << 24) & 0xFF0000000000) | ((data << 40) & 0xFF000000000000) |
			       (data << 56);
			double result;
		...
		if (is_first) {
			...
			if (has_z != geom_has_z) {
				throw SerializationException(
				    "WKB Reader: Z value presence does not match top level geometry, DuckDB requires all geometries "
				    "inside a WKB blob to have the same amount of coordinate dimensions");
			}
			if (has_m != geom_has_m) {
				throw SerializationException(
				    "WKB Reader: M value presence does not match top level geometry, DuckDB requires all geometries "
				    "inside a WKB blob to have the same amount of coordinate dimensions");
			}
		}
