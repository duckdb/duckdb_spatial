...
private:
    ...
    public:
        ...
        Properties(bool has_z, bool has_m) : data((has_z ? 1 : 0) | (has_m ? 2 : 0)),
            vertex_size(sizeof(double) * (2 + (has_z ? 1 : 0) + (has_m ? 1 : 0) )) { }
        bool HasZ() const { return (data & 1) != 0; }
        bool HasM() const { return (data & 2) != 0; }
        void SetZ(bool has_z) {
            data = (data & 2) | (has_z ? 1 : 0);
            vertex_size = sizeof(double) * (2 + (has_z ? 1 : 0) + (HasM() ? 1 : 0));
        }
        void SetM(bool has_m) { data = (data & 1) | (has_m ? 2 : 0);
...
public:
    ...
    VertexArray(Allocator &alloc_p, data_ptr_t vertex_data_p, uint32_t vertex_count_p, bool has_z, bool has_m)
        : alloc(alloc_p), vertex_data(vertex_data_p), vertex_count(vertex_count_p), owned_capacity(0), properties(has_z, has_m) { }
    ...
    VertexArray(Allocator &alloc_p, uint32_t owned_capacity_p, bool has_z, bool has_m)
        : alloc(alloc_p), vertex_data(alloc.get().AllocateData(owned_capacity_p * (sizeof(double) * (2 + (has_z ? 1 : 0) + (has_m ? 1 : 0)) ))),
    ...
    VertexArray(VertexArray &&other) noexcept
        : alloc(other.alloc), vertex_data(other.vertex_data), vertex_count(other.vertex_count), owned_capacity(other.owned_capacity),
            properties(other.properties) {
    ...
    void ReserveUnsafe(uint32_t count) {
        D_ASSERT(IsOwning()); // Only owning arrays can be modified
        D_ASSERT(count > vertex_count); // Cannot shrink the array
    ...
    void UpdateVertexType(bool has_z, bool has_m) {
        ...
        if(new_vertex_size > old_vertex_size) {
            vertex_data = alloc.get().ReallocateData(vertex_data, vertex_count * old_vertex_size, vertex_count * new_vertex_size);
            // Loop backwards
