...
public:
    ...
    operator TYPE() const noexcept { return Load<TYPE>(ptr_); }
    MisalignedRef& operator=(TYPE const& v) noexcept {
        Store<TYPE>(v, ptr_);
        return *this;
    }
    void reset(data_ptr_t ptr) noexcept { ptr_ = ptr; }
    data_ptr_t ptr() const noexcept { return ptr_; }
};
...
public:
    ...
    operator TYPE() const noexcept { return Load<TYPE>(ptr_); }
    const_data_ptr_t ptr() const noexcept { return ptr_; }
};
